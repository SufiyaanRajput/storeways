...
const FeaturesForm = ({type, setSectionToEdit, refetchLayout}) => {
  const [form] = useForm();

  useEffect(() => {
    form.setFields([{ name: 'sectionBgColor', value: type.sectionBgColor, errors: [] }, { name: 'features', value: type.items, errors: [] }]);
  ...
  const onSubmit = (features) => {
    reupdateSection({page: 'home', sectionId: type.id, ...features});
  };

  return (
    <Form name="features-form" 
      layout="vertical" 
      form={form}
      labelCol={{ span: 24 }}
      wrapperCol={{ span: 24 }} 
