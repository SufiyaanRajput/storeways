  ...
  return (
    <>
      <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
        <Item id={props.item.key} item={props.item} updateItemParent={props.updateItemParent} setSectionToEdit={props.setSectionToEdit}/>
      </div>
    </>
  );
}
  ...
  return(
      <SortableContext
        ...
        <div ref={setNodeRef} id={id}>
          {items.map((item) => (
            <SortableItem key={item.key} id={item.key} item={item} updateItemParent={updateItemParent} setSectionToEdit={setSectionToEdit}/>
          ))}
        </div>
      </SortableContext>
  );
...
const DragAndDrop = ({items, setItems, refetchLayout}) => {
  const [sectionToEdit, setSectionToEdit] = useState(null);
  const sections = ['CAROUSEL', 'BANNER', 'POSTERS', 'PRODUCTS', 'FEATURES', 'NEWSLETTER']
  ...
  return(
    <>
      <Drawer
        ...
        visible={sectionToEdit}
        size="large"
        key={"right"}
      >
        <SectionForm section={sectionToEdit} setSectionToEdit={setSectionToEdit} refetchLayout={refetchLayout}/>
      </Drawer>
      <Card>
      ...
      </DndContext>
      {
         <AddSectionCard>
          <Select onChange={onSelectNewSection} value={null} placeholder="Add new">
            {
              sections.map((item) => (<Select.Option key={item} value={item}>{item}</Select.Option>))
            }
          </Select>
       </AddSectionCard> 
      }
