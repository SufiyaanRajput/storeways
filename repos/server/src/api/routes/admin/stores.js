...
import { Store } from '@storeways/lib/domain';
...
router.post('/uploads/logo', auth(['owner']), upload.single('logo'), async (req, res) => {
  try{
    ...
    const { url, fileId } = await storage.upload(req.file.buffer, `${req.body.fileName}.${req.body.ext}`, 'uploads/logo');
...
router.post('/uploads/banner', auth(['owner']), upload.single('banner'), async (req, res) => {
  try{
    ...
    const storage = new FileStorage();
    const { url, fileId } = await storage.upload(req.file.buffer, `${req.body.fileName}.${req.body.ext}`, 'uploads/banners');
    res.status(200).send({banner: {
      url,
      fileId,
      name: req.body.fileName,
    }, message: 'Banner uploaded successfully!', success: true});
  }catch(error){
...
router.post('/uploads/poster', auth(['owner']), upload.single('poster'), async (req, res) => {
  try{
    ...
    const storage = new FileStorage();
    const { url, fileId } = await storage.upload(req.file.buffer, `${req.body.fileName}.${req.body.ext}`, 'uploads/posters');
    res.status(200).send({poster: {
      url,
      fileId,
      name: req.body.fileName,
    }, message: 'Poster uploaded successfully!', success: true});
  }catch(error){
