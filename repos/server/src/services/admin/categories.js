  ...
  if (categories.length) {
    ...
    const groupRelations = (source) => {
      let result = source.reduce((target, item) => {
        ...
        if (!target[item.name]) {
          target[item.name] = {categoryIds: [item.categoryId], id: item.id};
        } else {
          target[item.name].categoryIds.push(item.categoryId)
        }
    ...
    const groupedVariations = groupRelations(variations);
